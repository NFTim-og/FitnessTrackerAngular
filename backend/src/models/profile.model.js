...
class Profile {
  ...
  static async create(profileData) {
    try {
      ...
      await query(
        'INSERT INTO profiles (id, user_id, weight_kg, height_cm) VALUES (?, ?, ?, ?)',
        [profile.id, profile.user_id, profile.weight_kg, profile.height_cm]
      );
  ...
  static async update(userId, profileData) {
    try {
      ...
      await query(
        'UPDATE profiles SET weight_kg = ?, height_cm = ?, updated_at = ? WHERE user_id = ?',
        [profile.weight_kg, profile.height_cm, profile.updated_at, userId]
      );
